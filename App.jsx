...
const uploadProfilePhoto = async (file, userId) => {
  ...
  const { error } = await sb.storage.from('profile-pics').upload(path, file, { upsert: true });
  if (error) return null;
  const { data } = sb.storage.from('profile-pics').getPublicUrl(path);
  return data.publicUrl;
};

const renderPic = (pic, size=52, blur=false) => {
  const isUrl = pic && pic.startsWith("http");
  const style = {width:size,height:size,borderRadius:"50%",objectFit:"cover",filter:blur?"blur(8px)":"none",flexShrink:0};
  return isUrl
    ? <img src={pic} alt="profile" style={style}/>
    : <div style={{...style,background:"#f5f0ff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.5}}>{pic||"😊"}</div>;
};
...
.chat-bubble{max-width:75%;padding:12px 18px;border-radius:20px;margin:4px 0;font-size:14px;line-height:1.5;word-break:break-word;}
...
export default function App() {
  ...
  const [picMode, setPicMode] = useState("avatar");
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = sb.auth.onAuthStateChange((_event, session) => {
      if (session) loadUserProfile(session.user);
      else { setCurrentUser(null); setAuthUser(null); setScreen("signup"); }
    });
  ...
  const handleLogin = async () => {
    if (loginData.email === DEV.email && loginData.password === DEV.password) {
      setCurrentUser(DEV);
      setIsPremium({plan:"Dev Mode",price:"FREE"});
      const { data } = await sb.from("profiles").select("*");
      if (data) { setAllUsers(data); setLikes(data.slice(0,2)); }
      setScreen("main");
  ...
  const isCompat = (u) => {
    ...
    const myOrientation = currentUser?.orientation;
    const lookingFor = currentUser?.looking_for_gender;
    if (myOrientation === "straight") {
      if (myGender === "male") return u.gender === "female";
      if (myGender === "female") return u.gender === "male";
    }
    if (lookingFor && lookingFor !== "everyone") return u.gender === lookingFor;
    return true;
  };
...
const SETUP_STEPS = [
    ...
    { title: "Tell us about you! ✨", content: (
      <div>
        ...
        {profile.orientation && profile.orientation !== "straight" && (
          <div className="field">
            <label className="label">I am looking for</label>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {[...GENDERS,"everyone"].map(g => (
                <div key={g} className={`chip ${profile.lookingForGender===g?"active":""}`} onClick={() => setProfile(p=>({...p,lookingForGender:g}))}>{g}</div>
              ))}
            </div>
          </div>
        )}
  ...
  const chatList = Object.keys(chats).map(id => allUsers.find(u => u.id === id) || (currentUser?.isDevAccount ? allUsers[parseInt(id)-1] : null)).filter(Boolean);
            ...
            <div className="card">
              {!isLogin ? (
                ...
                <div>
                  ...
                  <div className="field"><label className="label">Email</label><input className="input" type="email" placeholder="your@email.com" value={loginData.email} onChange={e => setLoginData(p=>({...p,email:e.target.value}))}/></div>
                  <div className="field"><label className="label">Password</label><input className="input" type="password" placeholder="password" value={loginData.password} onChange={e => setLoginData(p=>({...p,password:e.target.value}))}/></div>
          ...
          <div style={{background:"white",padding:"20px 24px 16px",borderBottom:"2px solid #f0e6ff",position:"sticky",top:0,zIndex:50}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                {renderPic(currentUser?.profile_pic||currentUser?.profilePic,36)}
                <div>
                ...
                {matchResults.length === 0 ? (
                  ...
                  <div key={user.id} className="match-card" style={{border:user.isBoosted?"2px solid #ff6b00":"2px solid #f0e6ff",background:user.isBoosted?"linear-gradient(135deg,#fff8f0,#fff5f8)":"white"}}>
                    ...
                    <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
                      <div>
                        {renderPic(user.profile_pic,60,true)}
                        <div style={{textAlign:"center",fontSize:11,color:"#c44dff",fontWeight:700,marginTop:4}}>Hidden 🔒</div>
                      </div>
                      <div style={{flex:1,minWidth:0,overflow:"hidden"}}>
                        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"nowrap",minWidth:0}}>
                          <div style={{fontWeight:800,color:"#553c7b",fontSize:16,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",minWidth:0,flex:1}}>@{user.username} {isPremium?"✅":""}</div>
                          <div style={{fontSize:11,color:user.online?"#44ee88":"#bbb",fontWeight:700,whiteSpace:"nowrap",flexShrink:0}}>{user.online?"🟢 Online":"⚫ Offline"}</div>
                        </div>
...
{activeTab === "chats" && !activeChat && (
                ...
                {chatList.length === 0 ? (
                  ...
                  <div key={user.id} className="match-card" style={{display:"flex",alignItems:"center",gap:16,cursor:"pointer"}} onClick={() => startChat(user)}>
                    <div style={{position:"relative"}}>
                      {renderPic(user.profile_pic,52,!getReveal(user.id).theyShared)}
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:800,color:"#553c7b",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>@{user.username}</div>
                      <div style={{fontSize:13,color:"#a089c0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(chats[user.id]||[]).slice(-1)[0]?.text?.slice(0,40)}...</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {activeTab === "chats" && activeChat && (
              <div style={{paddingTop:8,overflowX:"hidden",maxWidth:"100%",boxSizing:"border-box"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,padding:"12px 0",borderBottom:"2px solid #f0e6ff",marginBottom:16,flexWrap:"nowrap",minWidth:0}}>
                  <button onClick={() => setActiveChat(null)} style={{background:"none",border:"none",cursor:"pointer",fontSize:20,color:"#c44dff",flexShrink:0}}>←</button>
                  {renderPic(activeChat.profile_pic,44,!getReveal(activeChat.id).theyShared)}
                  <div style={{flex:1,minWidth:0,overflow:"hidden"}}>
                    <div style={{fontWeight:800,color:"#553c7b",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>@{activeChat.username}</div>
                    <div style={{fontSize:12,color:activeChat.online?"#44ee88":"#bbb",fontWeight:600,whiteSpace:"nowrap"}}>{activeChat.online?"🟢 Online":"⚫ Offline"}</div>
                  </div>
                ...
                <div style={{background:"#f5f0ff",borderRadius:16,padding:16,marginBottom:16}}>
                  ...
                  <div style={{display:"flex",gap:10}}>
                    ...
                    <div style={{flex:1,background:"white",borderRadius:12,padding:10,textAlign:"center"}}>
                      ...
                      {getReveal(activeChat.id).theyShared?<div>{renderPic(activeChat.profile_pic,52)}</div>:<div style={{color:"#c44dff",fontWeight:700,fontSize:12}}>{getReveal(activeChat.id).iShared?"Waiting...":"🔒 Hidden"}</div>}
                    </div>
                  </div>
                </div>
                ...
                {getReveal(activeChat.id).theyShared && (
                  <div style={{background:"linear-gradient(135deg,#ffe0ee,#f0e6ff)",borderRadius:16,padding:16,marginBottom:16,textAlign:"center"}}>
                    <div style={{display:"flex",justifyContent:"center"}}>{renderPic(activeChat.profile_pic,80)}</div>
                    <p style={{color:"#8866bb",fontWeight:700,fontSize:14,marginTop:8}}>They shared their pic with you!</p>
                  </div>
                )}

                <div style={{display:"flex",flexDirection:"column",gap:8,minHeight:300,marginBottom:16,maxWidth:"100%",overflowX:"hidden"}}>
            ...
            {activeTab === "profile" && !editMode && (
              <div style={{paddingTop:20}}>
                <div style={{textAlign:"center",marginBottom:24,position:"relative"}}>
                  ...
                  <div style={{marginBottom:8,display:"flex",justifyContent:"center"}}>{renderPic(currentUser?.profile_pic||currentUser?.profilePic,90)}</div>
...
{activeTab === "profile" && editMode && (
              <div style={{paddingTop:20}}>
                ...
                <div className="card" style={{marginBottom:16}}>
                  <h3 style={{fontFamily:"'Fredoka One',cursive",color:"#553c7b",marginBottom:12}}>Profile Picture 📸</h3>
                  <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
                    {editData.profile_pic && editData.profile_pic.startsWith("http") ? (
                      <img src={editData.profile_pic} alt="profile" style={{width:90,height:90,borderRadius:"50%",objectFit:"cover",border:"3px solid #c44dff"}}/>
                    ) : (
                      <div style={{width:90,height:90,borderRadius:"50%",background:"#f5f0ff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48,border:"3px solid #c44dff"}}>{editData.profile_pic||editData.profilePic||"😊"}</div>
                    )}
                  </div>
                  <div style={{display:"flex",gap:10,marginBottom:16}}>
                    <button onClick={() => setPicMode("photo")} style={{flex:1,padding:"12px",borderRadius:14,border:picMode==="photo"?"2px solid #c44dff":"2px solid #f0e6ff",background:picMode==="photo"?"#f5f0ff":"white",fontWeight:700,color:picMode==="photo"?"#c44dff":"#a089c0",cursor:"pointer",fontFamily:"Nunito,sans-serif",fontSize:14}}>📷 Upload Photo</button>
                    <button onClick={() => setPicMode("avatar")} style={{flex:1,padding:"12px",borderRadius:14,border:picMode==="avatar"?"2px solid #c44dff":"2px solid #f0e6ff",background:picMode==="avatar"?"#f5f0ff":"white",fontWeight:700,color:picMode==="avatar"?"#c44dff":"#a089c0",cursor:"pointer",fontFamily:"Nunito,sans-serif",fontSize:14}}>🎭 Use Avatar</button>
                  </div>
                  {picMode === "photo" && (
                    <div>
                      <input ref={fileInputRef} type="file" accept="image/*" style={{display:"none"}} onChange={async (e) => {
                        ...
                        if (!file) return;
                        if (file.size > 5*1024*1024) return showNotif("Max file size is 5MB!", "error");
                        setPhotoUploading(true);
                        const url = await uploadProfilePhoto(file, currentUser.id);
                        setPhotoUploading(false);
                        if (url) { setEditData(p=>({...p,profile_pic:url})); showNotif("Photo uploaded! 📸"); }
                        else showNotif("Upload failed. Check storage settings.", "error");
                      }}/>
                      <button className="btn-primary" onClick={() => fileInputRef.current.click()} style={{marginBottom:8}}>{photoUploading?"Uploading... ⏳":"📷 Pick a Photo"}</button>
                      <p style={{fontSize:12,color:"#a089c0",textAlign:"center",fontWeight:600}}>Max 5MB · JPG, PNG, WebP</p>
                    </div>
                  )}
                  {picMode === "avatar" && (
